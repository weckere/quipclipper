...
def build_mkvmerge_args(
    *,
    ...
    if kind == "audio":
        # audio only: drop video, subtitles, attachments, buttons, chapters
        cmd += ["-D", "-S", "-M", "-B", "--no-chapters", "--no-global-tags"]
        if not all_audio:
            cmd += audio_sel
    else:  # video
        if not all_audio:
...
def cut_with_mkvmerge(
    ...
    *,
    ...
    args = build_mkvmerge_args(
        source=source, rng=rng, kind=kind, out=out, audio_ids=audio_ids,
        all_audio=all_audio, keep_subs=keep_subs,
        embed_subs=Path(embed_subs) if embed_subs else None,
    )
